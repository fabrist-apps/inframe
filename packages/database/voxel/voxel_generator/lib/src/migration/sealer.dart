...
final class VoxelArtifactSealer {
  Future<void> seal({required Directory directory, required String migrationId}) async {
    ...
    if (entries.last['id'] != migrationId) {
      throw const FormatException(
        'Only the latest unpublished migration may be sealed; correct settled history with a new migration.',
      );
    }
    ...
    final ranges = parseVoxelSqlStatements(sql);
    ...
    VoxelArtifactChecker().validateSealedMigration(sealedMigration, snapshot, sql);
